...
export const addtocart = async (req: any, res: any) => {
    ...
    if (existingcart) {
        ...
        if (existingcartitems) {
            const updatedQuantity = parseInt(existingcartitems.quantity) + parseInt(quantity);
            const updatedPrice = (parseInt(unitprice.replace(/,/g, '')) * updatedQuantity).toLocaleString('en-US');
            const { data: updatedCartItem, error: updateCartItemError } = await supabase.from('cart_items').update({ quantity: updatedQuantity, price: updatedPrice}).eq('id', existingcartitems.id).select().single();
        ...
        const { data: cartitem, error: cartitemerror } = await supabase.from('cart_items').insert({
            ...
            quantity,
            price: (parseInt(unitprice.replace(/,/g, '')) * quantity).toLocaleString('en-US'),
            size,
            unitprice
        }).select().single();
...
export const updatecartquantity = async (req: any, res: any) => {
    ...
    const {data:updatedcartitem,error:updatecartitemerror}=await supabase.from('cart_items').update({quantity:updatedQuantity,price:(parseInt(existingcartitem.unitprice.replace(/,/g, '')) * updatedQuantity).toLocaleString('en-US')}).eq('id',cartitemid).select().single();
    if(updatecartitemerror || !updatedcartitem){
        return res.status(500).json({message:"Error updating cart item quantity",updatecartitemerror})
    }
...
export const getcart=async(req:any,res:any)=>{
    const userid = req.user.id;
    const {data:existingcart,error:existingcarterror}=await supabase.from('carts').select(`*,user_id(firstname)`).eq('user_id',userid).single();
    if(existingcarterror || !existingcart){
        return res.status(404).json({message:"Cart not found",existingcarterror})
    }
    const {data:cartitems,error:cartitemserror}=await supabase.from('cart_items').select(`*,product_id(name,slug,image,description)`).eq('cart_id',existingcart.id);
    if(cartitemserror){
        return res.status(500).json({message:"Error retrieving cart items",cartitemserror})
    }
    return res.status(200).json({message:"Cart items retrieved successfully",cartitems,firstname:existingcart.user_id.firstname,})
}